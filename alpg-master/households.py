...
class Household:
    ...
    def simulate(self):
        for day in range(config.startDay, config.numDays + config.startDay):
            ...
            # Eliminate iron and vacuum cleaner
        ...
        if self.House.hasPV:
            ...
            self.PVProfile = self.Devices['PVPanel'].simulate(config.startDay,
                                                              config.numDays * ((3600 * 24) / config.timeBase),
...
class HouseholdFamilyDualWorker(Household):
    def __init__(self, parttime):
        self.generate()
        numKids = int(round(max(min(4, random.gauss(1.7, 0.4)),
...
class HouseholdFamilySingleParent(Household):
    def __init__(self, parttime):
        ...
        self.ConsumptionYearly = profilegentools.gaussMinMax(2500 + (700 * numKids), 500 + (
        numKids * 100)) * config.consumptionFactor  # kWh http://www.nibud.nl/uitgaven/huishouden/gas-elektriciteit-en-water.html
