...
class DeviceLighting(Device):
    def simulate(self, timeintervals, occupancy, timestamp):
        ...
        for m in range((sun['sunrise'].hour * 60 + sun['sunrise'].minute) + random.randint(-10, 40), \
                       (sun['sunset'].hour * 60 + sun['sunset'].minute) - random.randint(-10,
                                                                                         40)):  # Lighting quite well does match sunrise and sunset. Cloud data can enhance this. based on own experiences :)
...
class DeviceSolarPanel(Device):
    def calculateProduction(self, time, irradiation):
        ...
        azimuthRadian = self.azimuth * (
        math.pi / 180)  # in degrees: 0 is facing south, 90 is facing west, 180 is facing north and 270 east.
        ...
        E_time = 229.2 * (
        0.000075 + 0.001868 * math.cos(N) - 0.032077 * math.sin(N) - 0.014615 * math.cos(2 * N) - 0.04089 * math.sin(
            2 * N))

        local_std_time = int(((time / 60) % (24 * 60)))
        solar_time = local_std_time + 4 * (
        0 - longitudeRadian / (math.pi / 180)) + E_time;  # NOTE: DST screwed up here!
        omega = (((solar_time / 60) * 15) - 180) * (math.pi / 180)
        h = math.asin(
            math.cos(latitudeRadian) * math.cos(delta) * math.cos(omega) + math.sin(latitudeRadian) * math.sin(delta))
        ...
        if (k_T <= 0.0):
            ...
            I_d = irradiation * (
            0.9511 - 0.1604 * k_T + 4.388 * math.pow(k_T, 2) - 16.638 * math.pow(k_T, 3) + 12.336 * (math.pow(k_T, 4)))
        ...
        theta = math.acos(math.sin(delta) * math.sin(latitudeRadian) * math.cos(elevationRadian) -
                          math.sin(delta) * math.cos(latitudeRadian) * math.cos(azimuthRadian) * math.sin(
                              elevationRadian) +
                          math.cos(delta) * math.cos(latitudeRadian) * math.cos(omega) * math.cos(elevationRadian) +
                          math.cos(delta) * math.sin(latitudeRadian) * math.cos(azimuthRadian) * math.cos(
                              omega) * math.sin(elevationRadian) +
        ...
        while (time < (startday * 24 * 60 * 60) + timeintervals * config.timeBase):
            ...
            if (index > 0):
                ...
                if (offset < config.weather_timebaseDataset / 2):
                    irradiationNow = (0.5 - (offset / config.weather_timebaseDataset)) * irradiation[index - 1] + (
                                                                                                                  0.5 + (
                                                                                                                  offset / config.weather_timebaseDataset)) * \
                                                                                                                  irradiation[
                                                                                                                      index]
                else:
                    irradiationNow = (1.5 - (offset / config.weather_timebaseDataset)) * irradiation[index] + (-0.5 + (
                    offset / config.weather_timebaseDataset)) * irradiation[index + 1];
...
class DeviceWashingMachine(TimeShiftableDevice):
    def simulate(self, timeintervals, day, occupancy, washingMoment):
        washingtimeintervals = int(washingMoment - 30 + random.randint(0, 59))
        if (washingMoment < 0):
...
class DeviceDishwasher(TimeShiftableDevice):
    def simulate(self, timeintervals, day, occupancy, washingMoment):
        dishwashtimeintervals = int(washingMoment - 30 + random.randint(0, 59))
        if (washingMoment < 0):
...
class DeviceElectricalVehicle(BufferTimeshiftableDevice):
    def simulate(self, day, person, eventStart, eventDuration):
        ...
        if dayOfWeek in person.Workdays:
            ...
            energyLoss = round(energyLoss + (energyLoss * 0.166 * (
            (math.cos((day / 365) * 2 * math.pi)))))  # approx 25% less range in winter! Not considering heating here
